...
for sf in "${sfs[@]}"; do
        ...
        run_write_benchmark "sudo $CUSTOM -new" "xnvme DuckDB (Write) " "$sf" "$DEV"
        ...
        if echo "$sf <= 1" | bc -l | grep -q 1; then
                ...
                run_tpch_query "sudo $CUSTOM" "xnvme DuckDB (Test) tpch query 4" "4" "$sf" "$DEV"
        fi

        echo "===> READ-INTENSIVE QUERY BENCHMARK! RUNS: $RUNS"
        run_read_benchmark "$STANDARD" "Standard DuckDB (Read)" "$DB"
        run_read_benchmark "sudo $CUSTOM" "xnvme DuckDB (Read)" "$DEV"
