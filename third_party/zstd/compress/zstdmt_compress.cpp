...
static void ZSTDMT_freeFileBufferPool(ZSTDMT_bufferPool *bufPool) {
	DEBUGLOG(3, "ZSTDMT_freeFileBufferPool (address:%08X)", (U32)(size_t)bufPool);
	if (!bufPool)
...
static ZSTDMT_bufferPool *ZSTDMT_createFileBufferPool(unsigned maxNbBuffers, ZSTD_customMem cMem) {
	...
	if (bufPool->buffers == NULL) {
		ZSTDMT_freeFileBufferPool(bufPool);
		return NULL;
	}
...
static ZSTDMT_bufferPool *ZSTDMT_expandFileBufferPool(ZSTDMT_bufferPool *srcBufPool, unsigned maxNbBuffers) {
	if (srcBufPool == NULL)
		return NULL;
	...
	/* need a larger buffer pool */
	{
		...
		ZSTDMT_freeFileBufferPool(srcBufPool);
		newBufPool = ZSTDMT_createFileBufferPool(maxNbBuffers, cMem);
		if (newBufPool == NULL)
			return newBufPool;
...
static ZSTDMT_seqPool *ZSTDMT_createSeqPool(unsigned nbWorkers, ZSTD_customMem cMem) {
	ZSTDMT_seqPool *const seqPool = ZSTDMT_createFileBufferPool(SEQ_POOL_MAX_NB_BUFFERS(nbWorkers), cMem);
	if (seqPool == NULL)
		return NULL;
...
static void ZSTDMT_freeSeqPool(ZSTDMT_seqPool *seqPool) {
	ZSTDMT_freeFileBufferPool(seqPool);
}

static ZSTDMT_seqPool *ZSTDMT_expandSeqPool(ZSTDMT_seqPool *pool, U32 nbWorkers) {
	return ZSTDMT_expandFileBufferPool(pool, SEQ_POOL_MAX_NB_BUFFERS(nbWorkers));
}
...
static void ZSTDMT_serialState_update(serialState_t *serialState, ZSTD_CCtx *jobCCtx, rawSeqStore_t seqStore,
...
MEM_STATIC ZSTDMT_CCtx *ZSTDMT_createCCtx_advanced_internal(unsigned nbWorkers, ZSTD_customMem cMem,
	...
	mtctx->jobIDMask = nbJobs - 1;
	mtctx->bufPool = ZSTDMT_createFileBufferPool(BUF_POOL_MAX_NB_BUFFERS(nbWorkers), cMem);
...
size_t ZSTDMT_freeCCtx(ZSTDMT_CCtx *mtctx) {
	...
	ZSTDMT_freeJobsTable(mtctx->jobs, mtctx->jobIDMask + 1, mtctx->cMem);
	ZSTDMT_freeFileBufferPool(mtctx->bufPool);
...
static size_t ZSTDMT_resize(ZSTDMT_CCtx *mtctx, unsigned nbWorkers) {
	...
	mtctx->bufPool = ZSTDMT_expandFileBufferPool(mtctx->bufPool, BUF_POOL_MAX_NB_BUFFERS(nbWorkers));
...
static size_t ZSTDMT_flushProduced(ZSTDMT_CCtx *mtctx, ZSTD_outBuffer *output, unsigned blockToFlush,
...
size_t ZSTDMT_compressStream_generic(ZSTDMT_CCtx *mtctx, ZSTD_outBuffer *output, ZSTD_inBuffer *input,
