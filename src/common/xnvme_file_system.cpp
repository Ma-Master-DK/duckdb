...
void XNVMEFileSystem::Read(FileHandle &handle, void *buffer, int64_t nr_bytes, idx_t location) {
	...
	while (nr_bytes > 0) {
		SubmitRead(handle.path, &ctx, read_buffer, nr_bytes, location);
...
int64_t XNVMEFileSystem::Read(FileHandle &handle, void *buffer, int64_t nr_bytes) {
	...
	xnvme_cmd_ctx ctx;
	ctx = xnvme_cmd_ctx_from_dev(dev);
	SubmitRead(handle.path, &ctx, buffer, nr_bytes, 0);
	int64_t bytes_read = UnsafeNumericCast<int64_t>(ctx.cpl.result);
	return bytes_read;
}

void XNVMEFileSystem::SubmitRead(string &filepath, xnvme_cmd_ctx *ctx, void *buffer, int64_t nr_bytes, idx_t location) {
...
void XNVMEFileSystem::SubmitWrite(string &filepath, xnvme_cmd_ctx *ctx, void *buffer, int64_t nr_bytes,
                                  idx_t location) {
...
void XNVMEFileSystem::Write(FileHandle &handle, void *buffer, int64_t nr_bytes, idx_t location) {
	...
	while (nr_bytes > 0) {
		...
		SubmitWrite(handle.path, &ctx, write_buffer, UnsafeNumericCast<int64_t>(bytes_to_write), location);
...
int64_t XNVMEFileSystem::Write(FileHandle &handle, void *buffer, int64_t nr_bytes) {
	...
	while (nr_bytes > 0) {
		auto bytes_to_write = MinValue<idx_t>(idx_t(NumericLimits<int32_t>::Maximum()), idx_t(nr_bytes));
		SubmitWrite(handle.path, &ctx, write_buffer, UnsafeNumericCast<int64_t>(bytes_to_write),
		            UnsafeNumericCast<idx_t>(location));
