...
FileBlockHandle::~FileBlockHandle() { // NOLINT: allow internal exceptions
	...
	if (buffer && buffer_type != DBBufferType::TINY_BUFFER) {
		...
		buffer_manager.GetFileBufferPool().IncrementDeadNodes(*this);
	}
...
FileBufferHandle FileBlockHandle::LoadFromBuffer(BlockLock &l, data_ptr_t data, unique_ptr<FileBuffer> reusable_buffer,
	...
	readers = 1;
	memory_charge = std::move(reservation);
	return FileBufferHandle(shared_from_this(), buffer.get());
}

FileBufferHandle FileBlockHandle::Load(unique_ptr<FileBuffer> reusable_buffer) {
	...
	if (block_id < MAXIMUM_BLOCK) {
		...
		if (MustWriteToTemporaryFile()) {
			...
			return FileBufferHandle(); // Destroyed upon unpin/evict, so there is no temp buffer to read
		}
	}
	state = BlockState::BLOCK_LOADED;
	readers = 1;
...
unique_ptr<FileBuffer> FileBlockHandle::UnloadAndTakeBlock(BlockLock &lock) {
	VerifyMutex(lock);
