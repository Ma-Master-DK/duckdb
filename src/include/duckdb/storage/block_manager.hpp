...
public:
	...
	virtual unique_ptr<Block> ConvertBlock(block_id_t block_id, NvmeBuffer &source_buffer) = 0;
	virtual unique_ptr<Block> ConvertBlock(block_id_t block_id, DBBuffer &source_buffer) {
		if (auto buf = dynamic_cast<FileBuffer *>(&source_buffer)) {
			return ConvertBlock(block_id, *buf);
		} else if (auto buf = dynamic_cast<NvmeBuffer *>(&source_buffer)) {
			return ConvertBlock(block_id, *buf);
		} else {
	...
	virtual unique_ptr<Block> CreateBlock(block_id_t block_id, NvmeBuffer *source_buffer) = 0;
	virtual unique_ptr<Block> CreateBlock(block_id_t block_id, DBBuffer &source_buffer) {
		if (auto buf = dynamic_cast<FileBuffer *>(&source_buffer)) {
			return CreateBlock(block_id, *buf);
		} else if (auto buf = dynamic_cast<NvmeBuffer *>(&source_buffer)) {
			return CreateBlock(block_id, *buf);
		} else {
			throw IOException("Could not determine buffer type.");
	...
	//! Read the content of the block from disk
	virtual void ReadBlocks(DBBuffer &buffer, block_id_t start_block, idx_t block_count) = 0;

	//! Writes the block to disk
	virtual void Write(DBBuffer &block, block_id_t block_id) = 0;
